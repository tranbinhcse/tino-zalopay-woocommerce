...
class ZaloPayQr extends TinoZaloPayGateway {
	...
	public function __construct() {
        ...
        $this->method_title       = __( 'ZaloPay QR trên ZaloPay', 'woocommerce' );
        $this->method_description = 'Thanh toán quét QrCode tại site ZaloPay';
        $this->supports           = array(
          'subscriptions',
          'products',
        ...
        $this->method_description          = 'Thanh toán bằng thẻ QrCode cổng thanh toán ZaloPay';
        ...
        if ($zalostatus) {
           if ($zalostatus['returncode'] === 1) {
             ...
             if ($transID == $zalostatus['zptransid']) {
               $order->add_order_note( 'đã gọi IPN rồi ', false );
             } else {
               $total = round($order->get_total());
               $total = strval($total);
               if ($currency != 'VND') {
